...
```{r}
xgbcv <- xgb.cv( params = default_param, data = dtrain, nrounds = 1500, nfold = 5, showsd = T, stratified = T, print_every_n = 40, early_stopping_rounds = 10, maximize = F)
```
...
xgb_mod <- xgb.train(data = dtrain, params=default_param, nrounds = 1134)
```
...
```{r}
sub_data <- data.frame(Id = test_labels, SalePrice = (0.8*pred_XGB+0.2*pred_lasso))
